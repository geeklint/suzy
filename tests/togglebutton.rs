...
impl WidgetContent<OpenGlRenderPlatform> for GroupRoot {
    ...
    fn children<R: suzy::widget::WidgetChildReceiver<OpenGlRenderPlatform>>(&self, mut receiver: R) {
        receiver.child(&self.top);
        receiver.child(&self.middle);
        receiver.child(&self.bottom);
    }
...
const ALIGN_MASK: usize = usize::MAX ^ 3;
...
fn round_front(buffer: &[u8]) -> &[u8] {
    let start = (buffer.len() / 20) & ALIGN_MASK;
    &buffer[start..]
}

fn round_back(buffer: &[u8]) -> &[u8] {
    &buffer[..(buffer.len() * 19 / 20) & ALIGN_MASK]
}

fn round_both(buffer: &[u8]) -> &[u8] {
    let start = (buffer.len() / 20) & ALIGN_MASK;
    let end = (buffer.len() * 19 / 20) & ALIGN_MASK;
    &buffer[start..end]
}

fn is_color(buffer: &[u8], color: Color) -> bool {
